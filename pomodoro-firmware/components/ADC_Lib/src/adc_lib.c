...
bool adc_calibration_init(ADC_Config *config) {
    ...
    esp_err_t ret = adc_cali_create_scheme_curve_fitting(&cali_config, &config->calibration_handle);
    if (ret == ESP_OK) {
...
void adc_calibration_deinit(ADC_Config *config) {
    if (config->calibration_handle) {
        ESP_ERROR_CHECK(adc_cali_delete_scheme_curve_fitting(config->calibration_handle));
        config->calibration_handle = NULL;
    }
}
